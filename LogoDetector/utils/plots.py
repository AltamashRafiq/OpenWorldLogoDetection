...
def plot_one_box(x, img, color=None, label=None, line_thickness=None):
    ...
    c1, c2 = (int(x[0]), 100 + int(x[1])), (int(x[2]), 100 + int(x[3])) # AR added 100
...
def plot_one_box2(x, img, color=None, label=None, line_thickness=None):
    ...
    tl = line_thickness or round(0.002 * (img.shape[0] + img.shape[1]) / 2) + 1  # line/font thickness
    color = color or [random.randint(0, 255) for _ in range(3)]
    c1, c2 = (int(x[0]), int(x[1])), (int(x[2]), int(x[3])) # AR added 100
    cv2.rectangle(img, c1, c2, color, thickness=tl, lineType=cv2.LINE_AA)
    if label:
        tf = max(tl - 1, 1)  # font thickness
        t_size = cv2.getTextSize(label, 0, fontScale=tl / 3, thickness=tf)[0]
        ...
        if len(targets) > 0:
            ...
            for j, box in enumerate(boxes.T):
                ...
                if labels or conf[j] > 0.25:  # 0.25 conf thresh
                    ...
                    plot_one_box2(box, mosaic, label=label, color=color, line_thickness=tl)
